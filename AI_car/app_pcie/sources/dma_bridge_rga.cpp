...
extern "C" {
    ...
    int init_hardware_pipeline() {
     ...
     void sync_to_screen(uint8_t* hd_ptr) {
         ...
         im_rect src_rect = {0, 0, FPGA_IMG_WIDTH, FPGA_IMG_HEIGHT};
         im_rect drm_dst_rect = {0, 0, drm_ctx.mode.hdisplay, drm_ctx.mode.vdisplay};

         // 自适应竖屏：屏幕高 > 宽时，RGA 硬件旋转 90° 后缩放适配
         if (drm_ctx.mode.hdisplay < drm_ctx.mode.vdisplay) {
             im_opt_t opt = {};
             opt.core = IM_HAL_TRANSFORM_ROT_90;
             improcess(py_hd_buf, drm_dst_buf, {}, src_rect, drm_dst_rect, {}, 0, nullptr, &opt, 1);
         } else {
             improcess(py_hd_buf, drm_dst_buf, {}, src_rect, drm_dst_rect, {}, 0, nullptr, nullptr, 0);
         }
