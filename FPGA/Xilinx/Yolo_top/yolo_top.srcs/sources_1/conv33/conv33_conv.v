...
////////////////////////////////////////////////////输入九个weight的处理/////////////////////////////////////////////////////////////////
    ...
    wire [INT-1 : 0] weight_data[CHA_PAR_IN-1 : 0][CHA_PAR_OUT-1 : 0][0 : 2][0 : 2];
    ...
    generate
        for(genvar i = 0; i < CHA_PAR_IN; i = i + 1) begin: WIGHT_I
            for(genvar j = 0; j < CHA_PAR_OUT; j = j + 1) begin: WIGHT_j
                assign weight_data[i][j][0][0] = (r_ctl == 1'b0) ? weight_mem_ping[j][0][0][i*INT +: INT] : weight_mem_pang[j][0][0][i*INT +: INT];
                assign weight_data[i][j][0][1] = (r_ctl == 1'b0) ? weight_mem_ping[j][0][1][i*INT +: INT] : weight_mem_pang[j][0][1][i*INT +: INT];
                assign weight_data[i][j][0][2] = (r_ctl == 1'b0) ? weight_mem_ping[j][0][2][i*INT +: INT] : weight_mem_pang[j][0][2][i*INT +: INT];

                assign weight_data[i][j][1][0] = (r_ctl == 1'b0) ? weight_mem_ping[j][1][0][i*INT +: INT] : weight_mem_pang[j][1][0][i*INT +: INT];
                assign weight_data[i][j][1][1] = (r_ctl == 1'b0) ? weight_mem_ping[j][1][1][i*INT +: INT] : weight_mem_pang[j][1][1][i*INT +: INT];
                assign weight_data[i][j][1][2] = (r_ctl == 1'b0) ? weight_mem_ping[j][1][2][i*INT +: INT] : weight_mem_pang[j][1][2][i*INT +: INT];

                assign weight_data[i][j][2][0] = (r_ctl == 1'b0) ? weight_mem_ping[j][2][0][i*INT +: INT] : weight_mem_pang[j][2][0][i*INT +: INT];
                assign weight_data[i][j][2][1] = (r_ctl == 1'b0) ? weight_mem_ping[j][2][1][i*INT +: INT] : weight_mem_pang[j][2][1][i*INT +: INT];
                assign weight_data[i][j][2][2] = (r_ctl == 1'b0) ? weight_mem_ping[j][2][2][i*INT +: INT] : weight_mem_pang[j][2][2][i*INT +: INT];
            end
        end
    endgenerate
...
/////////////////////////////////// 加法输出 //////////////////////////////////////////////////////////
    ...
    generate
        for(genvar i = 0; i < CHA_PAR_IN; i = i+1) begin: ADD_PIPE_0_I
            for(genvar j = 0; j < CHA_PAR_OUT; j = j+1) begin: ADD_PIPE_0_J
                always @(posedge clk ) begin
                    //一拍
...
/////////////////////////////////// 数据信号 //////////////////////////////////////////////////////////
    ...
    assign m_valid = data_valid_d[DELAY-1] & data_valid_d[(DELAY-2)-1];
    //1拍数据延迟 + 4拍乘法器延迟 + 2拍3*3点加法延迟 + $clog2(CHA_PAR_IN)拍输入并行通道相加延迟     (1*1没有2拍3*3带你加法延迟)
    assign m_last  = data_last_d[(DELAY-2)-1];
