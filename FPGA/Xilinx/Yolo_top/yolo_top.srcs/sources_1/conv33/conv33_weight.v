module conv33_weight #(
    ...
    parameter CHA_PAR_IN = 16,                           //输入通道并行度
              ...
              WEIGHT_LEN = CHA_IMG_IN * CHA_IMG_OUT * 9 + CHA_IMG_OUT * 16,  //每次取一小部分weight
...
(
    ...
    input  [WEIGHT_LEN_WIDTH : 0] weight_len,              //这是向DMA请求weight的总字节数 包括bias   ===》 ci*co*9 + co * 16字节
    ...
    //         col_cnt <= 0;
    //     end
    //     else if((s_last & s_valid & s_ready) && (calculate_cnt == calculate_num - 1)) begin
    //         if(col_cnt == col_num_r - 1) begin
    //             calculate_cnt <= 0;
    //         end
    //         else begin
    ...
    always @(posedge clk) begin
        ...
        else if(((state == WDATA) && (next_state == W_WAIT)) && (calculate_cnt == calculate_num - 1)) begin
    ...
    wire [AXI_ADDR_WIDTH-1 : 0] weight_stride = WEIGHT_STRIDE_33;
    //内存读取地址更改
    ...
    always @(posedge clk) begin
        if(state == IDLE) begin
            ...
            if(data_addr == base_addr) begin        
                data_addr <= base_addr + bias_len + weight_stride;
            end
            else begin
                data_addr <= data_addr + weight_stride;
            end
        end
        else if((data_addr - base_addr) == weight_len) begin         //读取到循环地址  重新读取weight
    ...
    always @(posedge clk) begin
        if(state == IDLE) begin
            weight_len_r <= bias_len + weight_stride;
        end
        else if(cmd_valid & cmd_ready) begin
...
/////////////////////////////////////////从上级写入当前模块/////////////////////////////////////////
    ...
    localparam BRAM_BITS_TOTAL = 36864 * 2;
...
/////////////////////////////////////////从当前模块读出到下级/////////////////////////////////////////
    ...
    always @(posedge clk) begin
        ...
        else if(bias_cnt == (BISA_CNT_NUM - 1)) begin
            bias_ready <= 1'b0;
        end 
