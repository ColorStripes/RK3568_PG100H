            ...
            R_CENTER:begin
                if((calculate_cin_cnt == calculate_cin_num_r) && (calculate_cout_cnt == calculate_cout_num_r) && m_last) begin
                    if(row_cnt == (row_num - 1)) begin    //在执行最后一行的上一行后  状态要转移到最后一行
                        if(stride) begin                    //stride = 2时 行列数减半 是 向下取整   5-->2
                            next_state = IDLE;
                        end
                        else begin
                            next_state = R_LAST;
...
/////////////////////////////////////////从当前模块读出到下级/////////////////////////////////////////
    ...
    always @(posedge clk ) begin
        ...
        else if(stride) begin  //步长为2  组号每次+2
            if((state == W_CENTER) && (next_state == R_CENTER)) begin
