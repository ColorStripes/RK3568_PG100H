module conv11_out_buf_top #(
    ...
    parameter CONV_CHA_PAR_OUT = 8,
              ...
              DATA_DEPTH = (MAX_OUT_LEN / CHA_PAR_OUT) * (CHA_PAR_OUT / CONV_CHA_PAR_OUT),               //数据深度  这里是 W*channal/并行度  RAM总容量应该大于一行数据所需字节个数
...
(
    ...
    input  [CONV_CHA_PAR_OUT*INT-1 : 0]   s_data      ,
    input                                 s_valid     ,
    input                                 s_last      ,
    output                                s_req       ,
...
wire [CONV_CHA_PAR_OUT*INT-1 : 0] ping_s_data ;
wire                    ping_s_valid;
wire                    ping_s_last ;
wire                    ping_s_req  ;

wire [CONV_CHA_PAR_OUT*INT-1 : 0] ping_m_data ;
wire                    ping_m_valid;
wire                    ping_m_last ;
wire                    ping_m_req  ;
...
wire [CONV_CHA_PAR_OUT*INT-1 : 0] pang_s_data ;
wire                    pang_s_valid;
wire                    pang_s_last ;
wire                    pang_s_req;

wire [CONV_CHA_PAR_OUT*INT-1 : 0] pang_m_data ;
wire                    pang_m_valid;
wire                    pang_m_last ;
wire                    pang_m_req  ;
