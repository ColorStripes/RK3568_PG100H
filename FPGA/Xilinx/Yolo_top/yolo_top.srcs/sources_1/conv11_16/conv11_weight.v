...
(
    ...
    (*mark_debug = "true"*)input  [ROW_WIDTH : 0]         row_num          ,

    (*mark_debug = "true"*)input  [CALULATE_CNT_WIDTH : 0]      calculate_num         ,
    (*mark_debug = "true"*)input  [OUT_CALULATE_CNT_WIDTH : 0]  calculate_cout_num    ,
    

    (*mark_debug = "true"*)input  [DATA_WIDTH-1 : 0] s_data    ,
    (*mark_debug = "true"*)input                     s_valid   ,
    (*mark_debug = "true"*)input                     s_last    ,
    (*mark_debug = "true"*)output                    s_ready   ,

    

    (*mark_debug = "true"*)output reg [DATA_WIDTH-1 : 0] m_data_4  ,
    (*mark_debug = "true"*)output                        m_valid   ,
    (*mark_debug = "true"*)output                        m_last    ,
    (*mark_debug = "true"*)input                         m_req     ,
    ...
    (*mark_debug = "true"*)output [AXI_ADDR_WIDTH-1 : 0]  cmd_addr    ,
    (*mark_debug = "true"*)output [AXI_DATA_WIDTH-1 : 0]  cmd_len     ,
    (*mark_debug = "true"*)output                         cmd_valid   ,
    (*mark_debug = "true"*)input                          cmd_ready   ,

    (*mark_debug = "true"*)input          calculate_end                //计算结束信号4
    ...
    wire [RAM_CNT-1 : 0] almost_full, almost_empty, full, empty;
...
////////////////////////////////////////////计算次数/////////////////////////////////////////
    (*mark_debug = "true"*)reg [CALULATE_CNT_WIDTH : 0] calculate_cnt;
    always @(posedge clk) begin
        if(rst) begin
            calculate_cnt <= 0;
        end
    ...
    reg [AXI_ADDR_WIDTH-1 : 0] end_addr;
    always @(posedge clk) begin
        if(state == IDLE) begin
            end_addr <= base_addr + weight_sum; // 在启动时预先计算并打拍
        end
    end
    ...
    always @(posedge clk) begin
        ...
        else if(end_addr - data_addr >= type_stride) begin
            weight_stride <= type_stride;
        end
        else begin
            weight_stride <= end_addr - data_addr;
        end
    end
    ...
    //内存读取地址更改
    (*mark_debug = "true"*)reg [AXI_ADDR_WIDTH-1 : 0] data_addr;
    always @(posedge clk) begin
        if(state == IDLE) begin
            data_addr <= base_addr;
        end
        else if(cmd_valid & cmd_ready) begin
            if(data_addr + weight_stride == end_addr) begin       //读取到循环地址  重新读取weight 
...
/////////////////////////////////////////从上级写入当前模块/////////////////////////////////////////
    //写循环RAM计数器
    localparam RAM_CNT = 2,                                 //固定9个ram 每个ram存储9个点中一个
               RAM_CNT_WIDTH = $clog2(RAM_CNT+1);
    (*mark_debug = "true"*)reg [RAM_CNT_WIDTH-1 : 0] w_ram_cnt;
    always @(posedge clk ) begin
        if(rst)begin
            w_ram_cnt <= 4'd0;
        end
    ...
    (*mark_debug = "true"*)wire [RAM_CNT-1 : 0] wen;
    generate
    ...
    assign almost_full_now = (w_ram_cnt == 0) ? 1'b0 : almost_full[RAM_CNT-1];
    assign full_now = (w_ram_cnt == 0) ? 1'b0 : full[RAM_CNT-1];
    ...
    generate
        for(genvar i = 0; i < RAM_CNT; i = i + 1) begin
            ...
            sync_fifo #(                
                ...
                .ALMOST_EMPTY_NUM(CHA_PAR_OUT-1),
...
/////////////////////////////////////////从当前模块读出到下级/////////////////////////////////////////
    ...
    //读循环RAM计数器
    (*mark_debug = "true"*)reg [RAM_CNT_WIDTH-1 : 0] r_ram_cnt;
    always @(posedge clk ) begin
        if(rst) begin
            r_ram_cnt <= 4'd1;
        end
        ...
        else if((raddr == DEPTH_ADDR) && (r_ram_cnt == RAM_CNT) && r_valid) begin                   //下一轮写WEIGHT
            r_ram_cnt <= 4'd1;
        end 
    ...
    always @(posedge clk) begin
        if(rst) begin
            ...
            if(raddr == DEPTH_ADDR) begin
                ...
                for(i = 0; i < RAM_CNT; i = i + 1) begin
                    if(r_ram_cnt == (i+1) && ~almost_empty[i]) begin
                        ren[i] <= ren_all;
                    end
                end
            end
    ...
    //读地址
    (*mark_debug = "true"*)reg [ADDR_WIDTH-1 : 0] raddr;
    always @(posedge clk ) begin
        if(rst) begin
    ...
    assign r_valid = |ren;
    assign r_last = (raddr == CHA_PAR_OUT-1);     //所需要输出并行度的最后一个
