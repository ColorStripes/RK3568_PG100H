...
(
    ...
    //部分拿取的倍数 不能超过FIFO的深度
    // localparam MAX_MUL = 8;
    // wire [CALULATE_CNT_WIDTH : 0] multiple;
    // assign multiple = calculate_num > MAX_MUL ? MAX_MUL : calculate_num;
    ...
    // (*mark_debug = "true"*)wire [AXI_ADDR_WIDTH-1 : 0] weight_stride = WEIGHT_STRIDE_11 * multiple;
    (*mark_debug = "true"*)reg [AXI_ADDR_WIDTH-1 : 0] weight_stride;
    always @(posedge clk) begin
        weight_stride <= WEIGHT_STRIDE_11 * multiple;
    end
    ...
    always @(posedge clk) begin
        ...
        else if((data_addr - base_addr) == weight_sum) begin         //读取到循环地址  重新读取weight
            data_addr <= base_addr + bias_len;
        end
    end
    ...
    reg [AXI_DATA_WIDTH-1 : 0] weight_len_r;
    always @(posedge clk) begin
        if(state == IDLE) begin
            weight_len_r <= bias_len;
        end
        else if(cmd_valid & cmd_ready) begin
            weight_len_r <= weight_stride;
        end
    end
    assign cmd_len = weight_len_r;
