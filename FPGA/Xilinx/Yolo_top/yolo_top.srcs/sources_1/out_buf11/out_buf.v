module out_buf #(
    parameter CHA_PAR_OUT = 8,                          //输出通道并行度
              ...
              CONV_DATA_DEPTH = MAX_OUT_LEN / CONV_CHA_PAR_OUT,        //conv33的最大输出字节数 / CONV_CHA_PAR_OUT
              DATA_WIDTH = CHA_PAR_OUT * INT,           //数据传输位宽    输出并行度 * INT8
              DATA_DEPTH = 5120,                        //数据深度  这里是 W*channal/并行度  RAM总容量应该大于一行数据所需字节个数
              ADDR_WIDTH = DATA_DEPTH > CONV_DATA_DEPTH ? $clog2(DATA_DEPTH) : $clog2(CONV_DATA_DEPTH),          //地址位宽
...
(
    ...
    always @(posedge clk) begin
        if(rst) begin
            ...
            calculate_cnt <= calculate_cnt + 1;    //成功输出前并行度个通道数的一行
        end 
    end
    ...
    generate
        if (CONV_CHA_PAR_OUT == CHA_PAR_OUT) begin : gen_equal
            ...
            spram #(.DP(CONV_DATA_DEPTH),    //存储单元深度
                    .DW(CONV_DATA_WIDTH),    //数据位宽
                    .PIPE(READ_DELAY)  
            ) 
            sparm_inst_conv(
                .clk   (clk   ),
