module conv_weight #(
    ...
    parameter CHA_PAR_IN = 16,                           //输入通道并行度
              ...
              READ_DELAY = 1,
    //写循环RAM计数器
    parameter RAM_CNT = 9,                                 //固定9个ram 每个ram存储9个点中一个
    ...
    always @(posedge clk) begin
        if(rst) begin
            ...
            if(raddr == DEPTH_ADDR) begin
                for(k = 0; k < RAM_CNT; k = k + 1) begin
                    ren[k] <= 1'b0;
                end
            end
            else if(type) begin
                for(k = 0; k < RAM_CNT; k = k + 1) begin
                    if(r_ram_cnt == (k+1) && ~almost_empty[k]) begin
                        ren[k] <= ren_all;
                    end
                end
            end
