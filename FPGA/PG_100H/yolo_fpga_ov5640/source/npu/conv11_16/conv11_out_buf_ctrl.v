...
(
    ...
    //当前模块接受数据
    //CHA_PAR_OUT个输出
    input  [CONV_DATA_WIDTH-1 : 0]  s_data ,
    input                           s_valid,
    input                           s_last ,
    ...
    //ping接受数据
    output [CONV_DATA_WIDTH-1 : 0]  ping_s_data    ,
    output                          ping_s_valid  ,
    output                          ping_s_last   ,
    input                           ping_s_req  ,

    //ping向fifo输出数据
    input  [CONV_DATA_WIDTH-1 : 0]  ping_m_data   ,
    input                           ping_m_valid  ,
    input                           ping_m_last   ,
    output                          ping_m_req    ,


    output [CONV_DATA_WIDTH-1 : 0]  pang_s_data    ,
    output                          pang_s_valid  ,
    output                          pang_s_last   ,
    input                           pang_s_req    ,


    input  [CONV_DATA_WIDTH-1 : 0]   pang_m_data   ,
    input                            pang_m_valid  ,
    input                            pang_m_last   ,
    output                           pang_m_req    ,
...
///////////////////////////////计算信号相关////////////////////////////////////////
    //计算次数控制
    reg [CALULATE_CNT_WIDTH : 0]  calculate_cout_cnt;
    always @(posedge clk ) begin
        if(rst) begin
        ...
        // end
            ...
            if(calculate_cout_cnt == calculate_cout_num_r - 1) begin
                ...
                calculate_cout_cnt <= calculate_cout_cnt + 7'd1;
            end
        end
    end
