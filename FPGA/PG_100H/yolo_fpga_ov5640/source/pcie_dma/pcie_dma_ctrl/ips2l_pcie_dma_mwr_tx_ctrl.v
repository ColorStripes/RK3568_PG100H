...
reg     [63:0]  mwr_addr;
reg     [31:0]  mwr_data;
wire    [10:0]  mwr_length_tx_decode = (i_req_length == 11'd0) ? 11'd1024 : i_req_length;
wire    [10:0]  mwr_length_tx;
...
assign mwr_req_start = mwr_req_rcv && mwr_req_ack && |mwr_length_tx_decode;
...
begin
    if(!rst_n)
        ...
        mwr_length <= mwr_length_tx_decode;
    else if(mwr_length > {1'b0, max_payload_size} && state == HEADER_TX && !o_mwr_tx_hold) //header tx
        mwr_length <= mwr_length - {1'b0, max_payload_size};
    else if(mwr_length <= {1'b0, max_payload_size} && state == HEADER_TX && !o_mwr_tx_hold)
        mwr_length <= 11'd0;
end

//the true length to be transmitted
assign mwr_length_tx = (mwr_length > {1'b0, max_payload_size}) ? {1'b0, max_payload_size} : mwr_length;
...
assign mwr_header_tx    = {fmt,tlp_type,1'b0,tc,1'b0,attr[2],1'b0,th,td,ep,attr[1:0],at,mwr_length_tx[9:0]};

assign first_dwbe       = 4'hf;
assign last_dwbe        = mwr_length_tx == 11'h1 ? 4'h0 : 4'hf;
