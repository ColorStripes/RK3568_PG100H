...
generate
    if(M_A_DATA_WIDTH == 73)
        assign m_a_sign_ext = m_a;
    else
        assign m_a_sign_ext = {{{73-M_A_DATA_WIDTH}{m_a[M_A_DATA_WIDTH-1]}},m_a};
endgenerate

generate
    if(M_B_DATA_WIDTH == 82)
        assign m_b_sign_ext = m_b;
    else
...
genvar addr_i;
generate
    for(addr_i = 0; addr_i < 15; addr_i = addr_i + 1)
    begin:addr_gen
        if(addr_i == 0)
            assign addr[addr_i] = 4'b0;
        else
            assign addr[addr_i] = addr[addr_i-1] + 4'b1;
    end
endgenerate
