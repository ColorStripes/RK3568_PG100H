//SCCB ov5640寄存器配置模块
module ov5640_i2c_rgb565_cfg #(
      parameter   I2C_FREQ   = 18'd250_000     //IIC_SCL时钟频率
)
(  
    input                clk      ,     //模块驱动时钟 SCL的4倍频
    input                rst_n    ,     //复位信号，低电平有效
    
    input       [7:0]    i2c_data_r,    //I2C读出的数据
    input                i2c_done ,     //I2C读写完成信号
    input        [12:0]  cmos_h_pixel ,
    input        [12:0]  cmos_v_pixel ,
    input        [12:0]  total_h_pixel, //水平总像素大小
    input        [12:0]  total_v_pixel, //垂直总像素大小
    output  reg          i2c_exec ,     //I2C触发执行信号   
    output  reg  [23:0]  i2c_data ,     //I2C要配置的数据(高16位地址,低8位数据)
    output  reg          i2c_rh_wl,     //I2C读写控制信号
    output  reg          init_done      //初始化完成信号
);

    localparam DELAY_20MS_CNT = ((I2C_FREQ * 4) / 1000) * 21;
    localparam DELAY_20MS_CNT_WIDTH = $clog2(DELAY_20MS_CNT);

    localparam DELAY_5MS_CNT  = ((I2C_FREQ * 4) / 1000) * 5;   //5ms延时计数
    localparam DELAY_5MS_CNT_WIDTH = $clog2(DELAY_5MS_CNT);

    //parameter define
    localparam  REG_NUM = 8'd250  ;                    //需要配置的寄存器总数
    localparam  REG_NUM_WIDTH = $clog2(REG_NUM);       //需要配置的寄存器总数位宽

    //reg define
    reg   [DELAY_20MS_CNT_WIDTH : 0]   start_init_cnt;        //开始初始化延时计数器

    reg   [DELAY_5MS_CNT_WIDTH : 0]    delay_5ms_cnt ;        //5ms延时计数器
    reg                                delay_5ms_flag ;       //5ms延时标志位

    reg   [REG_NUM_WIDTH : 0]          init_reg_cnt  ;        //初始化寄存器计数器

    //*****************************************************
    //** main code
    //*****************************************************
    //SCL频率为250KHz时，系统clk频率为1Mhz，周期为1us 20000*1us = 20ms
    //OV5640上电后准备好SCCB通信至少需要20ms
    always @(posedge clk or negedge rst_n) begin
        if(!rst_n)
            start_init_cnt <= 1'b0;
        else if(start_init_cnt < DELAY_20MS_CNT) begin
    ...
    always @(posedge clk or negedge rst_n) begin
        if(!rst_n)
            delay_5ms_cnt <= 1'b0;
        else if(delay_5ms_flag && (delay_5ms_cnt < DELAY_5MS_CNT))
            delay_5ms_cnt <= delay_5ms_cnt + 1'b1;
        else
            delay_5ms_cnt <= 1'b0;
    end

    //5ms延时标志位，第2个寄存器配置完成后延时5ms
    always @(posedge clk or negedge rst_n) begin
        if(!rst_n)
            delay_5ms_flag <= 1'b0;
        else if(init_reg_cnt == 8'd3 && i2c_done)
            delay_5ms_flag <= 1'b1;
        else if(delay_5ms_cnt >= DELAY_5MS_CNT - 1'b1)
            delay_5ms_flag <= 1'b0;
    end
    ...
    always @(posedge clk or negedge rst_n) begin
        if(!rst_n)
            i2c_exec <= 1'b0;
        else if(start_init_cnt == DELAY_20MS_CNT - 1'b1)
            i2c_exec <= 1'b1;
        else if(delay_5ms_flag && (delay_5ms_cnt >= DELAY_5MS_CNT - 1'b1))
            i2c_exec <= 1'b1;
        else if(i2c_done && (init_reg_cnt < REG_NUM) && (init_reg_cnt != 8'd3))
            i2c_exec <= 1'b1;
        else
            i2c_exec <= 1'b0;
    end 
    ...
    always @(posedge clk or negedge rst_n) begin
        if(!rst_n)
            ...
            case(init_reg_cnt)
                ...
                8'd24 : i2c_data <= {16'h3600,8'h08}; //VCM控制,用于自动对焦
                8'd25 : i2c_data <= {16'h3601,8'h33}; //VCM控制,用于自动对焦
                8'd26 : i2c_data <= {16'h302d,8'h60}; //输出引脚控制
                8'd27 : i2c_data <= {16'h3620,8'h52};
                8'd28 : i2c_data <= {16'h371b,8'h20};
                8'd29 : i2c_data <= {16'h471c,8'h50};
                8'd30 : i2c_data <= {16'h3a13,8'h43};
                //AEC(自动曝光控制)
                8'd31 : i2c_data <= {16'h3a18,8'h00}; //AEC 目标范围
                8'd32 : i2c_data <= {16'h3a19,8'hf8}; //AEC 目标范围
                ...
                8'd50 : i2c_data <= {16'h4001,8'h02}; //BLC(黑电平补偿)控制
                8'd51 : i2c_data <= {16'h4005,8'h1a}; //BLC(黑电平补偿)目标值
                8'd52 : i2c_data <= {16'h3000,8'h00}; //系统寄存器复位
                8'd53 : i2c_data <= {16'h3004,8'hff}; //引脚控制
                8'd54 : i2c_data <= {16'h4300,8'h61}; //输出格式控制 RGB565
                ...
                //SDE(色彩深度扩展效应)配置 16'h5580~16'h558b
                ...
                //CIP(彩色图像处理)配置 (16'h5300~16'h530c)
                ...
                //时钟控制 Bit[7:4]:系统时钟源 input clock =24Mhz, PCLK = 48Mhz
                8'd205: i2c_data <= {16'h3035,8'h11};
                // 8'd206: i2c_data <= {16'h3036,8'h3c}; //PLL配置 48MHZ
                // 8'd206: i2c_data <= {16'h3036,8'h5A}; //PLL配置 72MHZ
                8'd206: i2c_data <= {16'h3036,8'h5F};    //PLL配置 76MHZ
                // 8'd206: i2c_data <= {16'h3036,8'h64};    //PLL配置 80MHZ
                8'd207: i2c_data <= {16'h3c07,8'h08};
                //分辨率设置 16'h3800~16'h3821
                ...
                8'd237: i2c_data <= {16'h4004,8'h02}; //BLC(黑电平补偿) 2 lines
                8'd238: i2c_data <= {16'h4713,8'h03}; //JPEG mode 3
                8'd239: i2c_data <= {16'h4407,8'h04}; //各种控制位
                ...
                8'd243: i2c_data <= {16'h5001,8'ha3}; //ISP 控制器
                8'd244: i2c_data <= {16'h3b07,8'h0a}; //闪光灯控制  
                //测试模式控制 
                8'd245: i2c_data <= {16'h503d,8'h00}; //8'h00:正常模式 8'h80:彩条测试模式
                //使能所有的ISP块
                8'd246: i2c_data <= {16'h3016,8'h02};
                8'd247: i2c_data <= {16'h301c,8'h02};
                8'd248: i2c_data <= {16'h3019,8'h02}; //休眠模式控制
                8'd249: i2c_data <= {16'h3019,8'h00}; //唤醒模式控制
                //注意,必须有case的默认分支，防止由于未知的状态导致死锁
                default : i2c_data <= {16'h300a,8'h00}; //读取ID高8位
            endcase
        end
    end
