module cat_add_para_top #(
    ...
    parameter CHA_PAR_IN = 16,                           //输入通道并行度
              ...
              CHA_IMG_OUT = CHA_IMG_IN * 2,                          //输入的IMG的最大通道数
              MAX_IN_CALULATE_NUM = (CHA_IMG_IN / CHA_PAR_IN),          //输入通道计算次数=输入通道数/输入并行度 
              MAX_OUT_CALULATE_NUM = (CHA_IMG_OUT / CHA_PAR_OUT),
...
(
    ...
    input  [OUT_LEN_WIDTH : 0]          out_col_channel_num, //out_col_channel_num = col * channel
    //输入通道计算次数
    input  [IN_CALULATE_CNT_WIDTH : 0]  calculate_cin_num  ,
    input  [OUT_CALULATE_CNT_WIDTH : 0] calculate_cout_num ,     //输出通道计算次数 
