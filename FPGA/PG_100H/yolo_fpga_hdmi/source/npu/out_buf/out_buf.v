module out_buf #(
    parameter CHA_PAR_OUT = 8,                          //���ͨ�����ж�
              ...
              INT = 8,                                  //ÿ������λ��
              MAX_OUT_LEN = 5120,                       //��ģ�������������ֽ���  Ҳ�������һ��*ͨ�����ֽ���
              CONV_DATA_WIDTH = CONV_CHA_PAR_OUT*INT,
              CONV_DATA_DEPTH = MAX_OUT_LEN / CONV_CHA_PAR_OUT,        //conv33���������ֽ��� / CONV_CHA_PAR_OUT
              DATA_WIDTH = CHA_PAR_OUT * INT,           //���ݴ���λ��    ������ж� * INT8
              DATA_DEPTH = MAX_OUT_LEN / CHA_PAR_OUT,                        //�������  ������ W*channal/���ж�  RAM������Ӧ�ô���һ�����������ֽڸ���
              ADDR_WIDTH = DATA_DEPTH > CONV_DATA_DEPTH ? $clog2(DATA_DEPTH) : $clog2(CONV_DATA_DEPTH),          //��ַλ��
              READ_DELAY = 1                            //������������Ҫ���ӳ�
)
(
    input          clk,
    ...
    generate
        if (CONV_CHA_PAR_OUT == CHA_PAR_OUT) begin : gen_equal

            assign m_data = rdata;

        end 
        else begin : gen_unequal

            wire [CONV_DATA_WIDTH-1 : 0] rdata_conv;
            spram #(.DP(CONV_DATA_DEPTH),    //�洢��Ԫ���
                    .DW(CONV_DATA_WIDTH),    //����λ��
                    .PIPE(READ_DELAY)  
            ) 
            sparm_inst_conv(
                .clk   (clk   ),
                .wdata (wdata[CONV_DATA_WIDTH-1 : 0]),
                .wen   (wen   ),
                .waddr (waddr ),
                .ren   (ren   ),
                .raddr (raddr ),
            ...
            assign m_data = conv_start ? {{DATA_WIDTH-CONV_DATA_WIDTH{1'b0}}, rdata_conv} : rdata;
