...
module pango_pcie_top #(
    ...
    // //*********************************************************************  
    output  wire     [31:0]           fpga_ddr3_addr         ,  //ddr3要操作的真实地址
    output  wire     [31:0]           fpga_ddr3_length       ,  //要操作的长度
    output  wire     [31:0]           fpga_ddr3_total_length ,
    //FPGA --> CPU
    input                             fpga_tx_done           ,             //empty上升沿 已经把数据从FIFO搬运到CPU的LPDDR4
    output                            mwr_tx_busy            ,             //该信号上升沿 需要把数据从FIFO搬运进来    
    input                             fpga_data_ready        ,           // 外部数据就绪信号（DDR3 写完成后通知 FPGA 可以开始读取数据）
    output                            fpga_tx_clk_en         ,           //数据进来的使能 
    output        [ADDR_WIDTH-1:0]    fpga_tx_addr           ,           //没用
    input         [127:0]             fpga_tx_data           ,           //进来的数据

    //CPU --> FPGA
    input                             fpga_rx_done           ,             //empty上升沿 已经把数据从FIFO搬运到FPGA的DDR3
    output                            mrd_rx_busy            ,              //该信号下降沿 可以把数据从RAM搬运到FIFO
    input                             fpga_rx_clk_en         ,           //从ram搬运数据使能
    input         [ADDR_WIDTH-1:0]    fpga_rx_addr           ,           //读取ram的地址
    output        [127:0]             fpga_rx_data           ,           //从ram中读到的数据

    output                            fifo_pcie_clk          ,
    output                            fifo_pcie_rst

);

assign fifo_pcie_clk = pclk_div2;
assign fifo_pcie_rst = ~core_rst_n;
...
(
    ...
    ////////////
    .o_fpga_msi             (dma_msi_req              ),
    .i_fpga_msi_grant       (dma_msi_grant            ),
    .o_dma_active           (dma_active               ),    
    //*********************************************************************
    .o_ddr3_addr  (fpga_ddr3_addr  )             ,
    .o_ddr3_length(fpga_ddr3_length)             ,
    .o_ddr3_total_length(fpga_ddr3_total_length) ,
    //FPGA --> CPU
    .fpga_tx_done(fpga_tx_done),
    .mwr_tx_busy(mwr_tx_busy),    
    // [新增] 外部数据就绪信号
    .i_data_ready (fpga_data_ready),
    .fpga_tx_clk_en(fpga_tx_clk_en),
    .fpga_tx_addr  (fpga_tx_addr  ),
    .fpga_tx_data  (fpga_tx_data  ),

    //CPU --> FPGA
    .fpga_rx_done   (fpga_rx_done),
    .mrd_rx_busy    (mrd_rx_busy),
    .fpga_rd_clk_en(fpga_rx_clk_en),
    .fpga_rd_addr  (fpga_rx_addr  ),
    .fpga_rd_data  (fpga_rx_data  )
);
...
// MSI Interrupt Logic
//**********************************************************************
wire                     cfg_msi_en;     // 接收来自 PCIe IP 的 MSI 使能状态 (CPU是否允许MSI)
wire                     ven_msi_req;    // 输出给 PCIe IP 的中断请求
wire                     ven_msi_grant;  // 接收来自 PCIe IP 的应答信号
wire     [MSI_NUM-1 : 0] ven_msi_vector;
wire     [31 : 0]        cfg_msi_pending; 

wire     [MSI_NUM-1 : 0] msi_req  ;      
wire     [MSI_NUM-1 : 0] msi_grant;

assign msi_req = {i_msi_req, dma_msi_req}  ;

assign dma_msi_grant = msi_grant[0];
assign o_msi_grant = msi_grant[MSI_NUM-1 : 1]; 


msi_arbiter #(
    .MSI_NUM(MSI_NUM)  // MSI中断数量
)
msi_arbiter(
    .clk(pclk_div2),              // 时钟信号
    .rst(!core_rst_n),              // 异步复位信号（高有效）

    .cfg_msi_en(cfg_msi_en), // 总中断使能信号（来自主机配置空间）
    .dma_active(dma_active), // DMA占用标志（为1时仅允许msi[0]通过）

    .msi_req(msi_req),           // MSI中断请求输入（电平信号，可能一直拉高）
    .msi_grant(msi_grant),     // 返回给各模块的中断grant信号

    .ven_msi_req  (ven_msi_req  ),   // 输出的中断请求信号（高有效）
    .ven_msi_grant(ven_msi_grant), // 主机侧返回的中断接收确认信号（1bit脉冲）
    .ven_msi_vector(ven_msi_vector), // 当前仲裁选中的中断向量号
    
    .cfg_msi_pending(cfg_msi_pending)  // 主机侧读取的挂起中断状态寄存器
);
...
(
    ...
    .cfg_msi_pending            (cfg_msi_pending        ),      //input   [(32*1)-1:0]
    .cfg_msi_en                 (cfg_msi_en             ),      //output
    
    .ven_msi_req                (ven_msi_req            ),      //input
    .ven_msi_tc                 (3'b000                 ),      //input   [2:0]
