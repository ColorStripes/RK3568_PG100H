...
wire    [10:0]  mrd_length_tx_decode = (i_req_length == 11'd0) ? 11'd1024 : i_req_length;

reg     [63:0]  mrd_addr;
reg     [10:0]  mrd_length;
reg     [10:0]  mrd_length_ff;
wire    [10:0]  mrd_length_tx;
...
assign mrd_req_start = mrd_req_rcv && mrd_req_ack && |mrd_length_tx_decode;
...
begin
    if(!rst_n)
        ...
        mrd_length <= mrd_length_tx_decode;
    else if((mrd_length > {1'b0, max_rd_req_size}) && tx_mrd && i_axis_slave1_trdy)
        mrd_length <= mrd_length - {1'b0, max_rd_req_size};
    else if((mrd_length <= {1'b0, max_rd_req_size}) && tx_mrd && i_axis_slave1_trdy)
        mrd_length <= 11'b0;
end
...
assign mrd_length_tx = (mrd_length_ff > {1'b0, max_rd_req_size}) ? {1'b0, max_rd_req_size} : mrd_length_ff;
...
assign mrd_header_tx    = {fmt,tlp_type,1'b0,tc,1'b0,attr[2],1'b0,th,td,ep,attr[1:0],at,mrd_length_tx[9:0]};
assign first_dwbe       = 4'hf;
assign last_dwbe        = mrd_length_tx == 11'h1 ? 4'h0 : 4'hf;
...
// 当 tx_busy 为 1 (还在发读请求) 或 cpld_tag 不全为 0 (还在等读响应) 时，都视为 RX 忙碌
assign o_mrd_rx_busy = tx_busy || (|cpld_tag);
//debug_bus
//assign o_dbg_bus = {
//                    tlp_tx_sum //13:0
//                    };
