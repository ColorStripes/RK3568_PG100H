...
DEFAULT_DET_MODEL = '/home/linaro/workspace/rknn/INT8_single_plate_640_0.5.rknn'
...
DEFAULT_HW_LIB = '/home/linaro/workspace/AI_car_pro/libdma_rga_camera.so'
DEFAULT_HDMI_LIB = '/home/linaro/workspace/AI_car_pro/libdma_rga.so'

# ── 小车控制库默认路径 ──
DEFAULT_CAR_LIB = '/home/linaro/workspace/AI_car_pro/libdma_rga.so'
DEFAULT_CAR_PORT = 8888

HW_YOLO_SIZE = 640 * 640 * 3
HW_HD_SIZE = 1280 * 720 * 3
HW_HDMI_SIZE = 1920 * 1080 * 3
...
class MotorOverPCIe:
    ...
    def headlight_on(self):
        self._lib.gpio(self.HEADLIGHT, 1)

    def headlight_off(self):
        self._lib.gpio(self.HEADLIGHT, 0)

    def left_signal_on(self):
        self._lib.gpio(self.LEFT_SIGNAL, 1)

    def left_signal_off(self):
        self._lib.gpio(self.LEFT_SIGNAL, 0)

    def right_signal_on(self):
        self._lib.gpio(self.RIGHT_SIGNAL, 1)

    def right_signal_off(self):
        self._lib.gpio(self.RIGHT_SIGNAL, 0)

    def horn(self, duration_ms=500):
        self._lib.gpio(self.HORN_PIN, 1)
        time.sleep(duration_ms / 1000.0)
        self._lib.gpio(self.HORN_PIN, 0)

    def buzzer_beep(self, count=1, interval_ms=100):
        for _ in range(count):
            self._lib.gpio(self.BUZZER_PIN, 1)
            time.sleep(interval_ms / 1000.0)
            self._lib.gpio(self.BUZZER_PIN, 0)
            if count > 1:
                time.sleep(interval_ms / 1000.0)
    ...
    def emergency_stop_all(self):
        self.stop()
        self.headlight_off()
        self.left_signal_off()
        self.right_signal_off()

    def drive_circle(self, duty, duration_ms):
        self.turn_right(duty)
        time.sleep(duration_ms / 1000.0)
        self.stop()

    def drive_slalom(self, duty, cycles=3):
...
class PlateSlotTracker:
    ...
    def _text_similar(a, b):
        """两个车牌核心号是否足够相似（允许 ≤1 个字符误识别，忽略空格差异）"""
        if not a or not b:
            return False
        a = a.replace(' ', '')
        b = b.replace(' ', '')
...
class PlateRecognitionWindow(QMainWindow):
    ...
    # ── TTS 播报参数 ──
    TTS_MIN_CONSECUTIVE = 3    # 连续出现 N 帧后才播报（过滤 OCR 抖动）
    ...
    def _start_sdcard(self):
        ...
        self._sdcard_thread = threading.Thread(target=self._sdcard_worker, daemon=True)
    ...
    def _sdcard_worker(self):
        ...
        # ====================================================================
        AI_SKIP = 1                       # 每 (AI_SKIP+1) 帧做一次 AI

        ai_state = {
            'frame': None,                # 待 AI 处理的 BGR 帧（原分辨率，copy）
            'ready': False,
        ...
        render_state = {
            'frame': None,                # 待渲染的 BGR 帧（主循环 copy，安全可变）
            'results': [],                # 对应的 AI 结果
        }
        ...
        def ai_thread_func():
            ...
            while not ai_exit.is_set():
                ...
                try:
                    ...
                    yolo_buf[0] = cv2.resize(
                        cv2.cvtColor(frame_bgr, cv2.COLOR_BGR2RGB), (640, 640))
                    outputs = self.rknn_det.inference(inputs=[yolo_buf])
                    boxes_list, scores_list = decode_yolo_fast(outputs, ow, oh)
        ...
        def render_thread_func():
            ...
            while not render_exit.is_set():
                ...
                results = []
                with render_lock:
                    if render_state['frame'] is not None:
                        frame_bgr = render_state['frame']
                        results = list(render_state['results'])

                if frame_bgr is None:
                    continue

                # ── 1. resize 到 1280×720 BGR ──
                ...
                for res in results:
                    ...
                    cv2.rectangle(resize_buf, (x1, y1), (x2, y2), res['color_bgr'], 3)
                    ...
                    if roi_y2 > roi_y1 and roi_x2 > roi_x1:
                        roi = resize_buf[roi_y1:roi_y2, roi_x1:roi_x2].copy()
                        roi_rgb = cv2.cvtColor(roi, cv2.COLOR_BGR2RGB)
                        roi_pil = Image.fromarray(roi_rgb)
                        draw_pil = ImageDraw.Draw(roi_pil)
                        draw_pil.text((0, 0), res['text'], font=self.font, fill=res['color_rgb'])
                        resize_buf[roi_y1:roi_y2, roi_x1:roi_x2] = cv2.cvtColor(
                            np.array(roi_pil), cv2.COLOR_RGB2BGR)

                # ── 3. 旋转 90° CW：1280×720 → 720×1280（图像+标注一起转）──
                rotated = cv2.rotate(resize_buf, cv2.ROTATE_90_CLOCKWISE)

                # ── 4. RGA 上屏：BGR888→BGRA8888 + 缩放至屏幕（无旋转）──
        ...
        try:
            while not self._stop_sdcard_flag:
                ...
                # ── 投喂 AI 线程（跳帧，copy 保证线程安全）──
                ai_frame_idx += 1
                if ai_frame_idx % (AI_SKIP + 1) == 0:
                    with ai_lock:
                        if not ai_state['ready']:
                            ai_state['frame'] = frame_bgr.copy()
                            ai_state['ready'] = True
                            ai_state['feed_frame_id'] = ai_frame_idx

                # ── 取最新 AI 结果 ──
                with ai_lock:
                    latest_results = list(ai_state['latest_results'])

                # ── 投喂渲染线程（每帧 copy，渲染线程会修改像素）──
                with render_lock:
                    render_state['frame'] = frame_bgr.copy()
                    render_state['results'] = latest_results
                render_event.set()
    ...
    def _sdcard_worker_image(self, path):
        ...
        try:
            ...
            if outputs is not None:
                boxes_list, scores_list = decode_yolo_fast(outputs, orig_w, orig_h)
                if len(boxes_list) > 0:
                    indices = cv2.dnn.NMSBoxes(boxes_list, scores_list, CONF_THRESH, 0.45)
                else:
                    indices = np.array([])
    ...
    def _tts_maybe_announce(self, slots_data):
        ...
        for plate in current_plates:
            if plate not in self._tts_consecutive:
                ...
                self._tts_consecutive[plate] = self.TTS_MIN_CONSECUTIVE if was_empty else 1
    ...
    def _load_models(self):
        ...
        for path, name in [(det_path, "检测模型"), (rec_b4_path, "B4识别模型"), (rec_b1_path, "B1识别模型")]:
    ...
    def _release_models_internal(self):
        for attr in ['rknn_det', 'rknn_rec_b4', 'rknn_rec_b1']:
    ...
    def _start_hdmi(self):
        ...
        self._hdmi_thread = threading.Thread(target=self._camera_worker_hdmi, args=(connector_idx,), daemon=True)
        ...
        try:
            ...
            while not self._stop_hdmi_flag:
                ...
                if elapsed >= 1.0:
                    ...
                    fps_timer = now
                    self.fps_signal.emit(fps_display)
                    set_status(f"HDMI运行中 | 检测到 {len(current_results)} 个车牌 | {fps_display:.0f}fps", '#009900')
    ...
    def _camera_worker_usb(self):
        ...
        try:
            ...
            while not self._stop_camera_flag:
                ...
                boxes_list, scores_list = decode_yolo_fast(outputs, img_w, img_h)
                indices = cv2.dnn.NMSBoxes(boxes_list, scores_list, CONF_THRESH, 0.45)
    ...
    def closeEvent(self, event):
        if self.camera_running or self.hdmi_running or self.sdcard_running:
