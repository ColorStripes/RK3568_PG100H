...
//#include <glib.h>     // glib 库，当前注释掉未使用
// #include <gtk/gtk.h>    // GTK 图形界面库
//#include "color.h"    // 颜色相关头文件，当前注释掉未使用
...
#define PCI_GET_NPU                     _IOWR(TYPE, 15, int)
#define PCI_ADC_READ_CMD                _IOWR(TYPE, 15, int)
#define PCI_SPI_BUS_CMD                 _IOWR(TYPE, 16, int)
#define PCI_I2C_BUS_CMD                 _IOWR(TYPE, 17, int)
#define PCI_CAN_BUS_CMD                 _IOWR(TYPE, 18, int)
#define PCI_UART_SEND_CMD               _IOWR(TYPE, 19, int)
#define PCI_IMU_READ_CMD                _IOWR(TYPE, 20, int)
#define PCI_ULTRASONIC_CMD              _IOWR(TYPE, 21, int)
...
typedef struct _ULTRASONIC_DATA_ {
...
typedef struct _SENSOR_FUSION_ {
    IMU_DATA imu;
    ULTRASONIC_DATA ultrasonic[4];
    unsigned int battery_mv;
    unsigned int motor_current_ma;
    unsigned int chip_temp_c;
    unsigned char collision_warning;
    unsigned char lane_departure;
    unsigned char reserved[2];
} SENSOR_FUSION;

int open_pci_driver(void);
void cmd_operation(int fd, unsigned char cmd, COMMAND_OPERATION *cmd_op);
int pci_map(int pci_driver_fd, int cmd, unsigned int total_size_tx, unsigned int total_size_rx);
int pci_umap(int pci_driver_fd);
DMA_DATA pci_mmp(int pci_driver_fd, int cmd, unsigned int total_size_tx, unsigned int total_size_rx);
int pci_dma_single_write(int pci_driver_fd, unsigned int start_ddr3_addr, unsigned int mmp_offset, unsigned int total_size);
int pci_dma_single_read(int pci_driver_fd, unsigned int start_ddr3_addr, unsigned int mmp_offset, unsigned int total_size);
