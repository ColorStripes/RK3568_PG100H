...
// // NPU控制参数结构体 — 与内核驱动 pango_pci_driver.h 保持一致
// typedef struct _NPU_CONFIG_ {
//     unsigned int instr_ddr3_addr;    // FPGA DDR3 上 NPU 指令存储起始地址 (例如 0x40000000)
//     unsigned int instr_total_size;   // 指令数据总字节数
//     unsigned int output_ddr3_addr;   // FPGA DDR3 上 NPU 输出数据起始地址
//     unsigned int output_total_size;  // NPU 输出数据总字节数
//     unsigned int output_host_offset; // 主机 RX 缓冲区中存放 NPU 输出的偏移
//     unsigned int tx_buf_offset;      // TX 缓冲区内偏移 (摄像头控制数据占用预留)
// } NPU_CONFIG;
