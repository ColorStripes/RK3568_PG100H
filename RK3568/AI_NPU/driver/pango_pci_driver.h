...
#define PCI_GET_NPU						_IOWR(TYPE, 15, int)						/* 获取NPU推理完成信号（中断驱动） */
...
// =====================================================================
// NPU 输出参数 — 与 main_npu.cpp 保持一致，修改类别数时两边同步
// =====================================================================
#define NPU_NUM_CLASSES      4           // 检测类别数量
#define NPU_BBOX_PARAMS      5           // 每个 anchor: cx, cy, w, h, conf
#define NPU_NUM_ANCHORS      3           // YOLO anchor 数量
#define NPU_FPGA_CH          32          // FPGA 硬核通道数 (≥ EFFECTIVE_CH, 16的倍数)

// 自动计算 (不要手动改)
#define NPU_EFFECTIVE_CH      (NPU_NUM_ANCHORS * (NPU_BBOX_PARAMS + NPU_NUM_CLASSES))  // 3*(5+4)=27
#define NPU_PAD_CHANNELS      (NPU_FPGA_CH - NPU_EFFECTIVE_CH)                         // 5

// 三头输出总字节数 = (20² + 40² + 80²) × 32
...
C_GPIO c_gpio;
#define MAX_PWM_CHANNELS 256
static struct pwm_device *g_pwm_devs[MAX_PWM_CHANNELS] = {NULL};
