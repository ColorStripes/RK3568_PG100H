...
// ---- 主机 DMA 缓冲区大小 ----
// TX: 3MB = 前 256KB 相机保留区 + NPU 区(层数/启动/权重/图像/指令), 图像区 0x190000 字节
#define TX_BUF_SIZE             (3 * 1024 * 1024UL)
#define RX_BUF_SIZE             (10 * 1024 * 1024UL)

// ---- FPGA DDR3 地址 ----
#define NPU_INSTR_DDR3_BASE     0x40000000
#define NPU_START_DDR3_BASE     0x51000000
#define NPU_WEIGHT_DDR3_BASE    0x02000000
#define NPU_WEIGHT_STRIDE       0x00100000
#define NPU_IMAGE_DDR3_ADDR     0x06000000

// ---- 输入图像 (RGBA 640×640, 4B/px 隔行; FPGA focus_in_buf 丢弃 alpha) ----
#define NPU_INPUT_W             640
#define NPU_INPUT_H             640
#define NPU_INPUT_C             4
#define NPU_IMAGE_SIZE          (NPU_INPUT_W * NPU_INPUT_H * NPU_INPUT_C)

// ---- 输入量化参数 (yolov5/npy/fpga_params.h: quant.scale / quant.zero_point) ----
#define NPU_QUANT_SCALE         0.00391965
#define NPU_QUANT_ZERO          0

// ---- letterbox 填充灰阶 (与 yolov5/utils/utils.py resize_image 一致) ----
#define NPU_LETTERBOX_PAD       128

// ---- 主机 TX 缓冲区分区 (后 2.5MB, 互不重叠) ----
#define TX_CAMERA_RESERVED      (256 * 1024)
...
// 0x142000 起, 0x190000 字节 RGBA 图像; 指令流从 0x2D2000 起
