...
if __name__ == "__main__":
    ...
    #----------------------------------------------------------------------------------------------------------------------------#
    model_path      = 'logs/ep300-loss0.019-val_loss0.019.pth'
    #------------------------------------------------------#
    ...
    def get_dataloader_workers():
        if distributed and ngpus_per_node > 1:
            return max(2, num_workers // ngpus_per_node)
        return num_workers
    ...
    #------------------------------------------------------#
    train_annotation_path   = os.path.join(DATA_ROOT, 'CCPD_CRPD_train.txt')
    val_annotation_path     = os.path.join(DATA_ROOT, 'CCPD_CRPD_val.txt')
    ...
    if not pretrained:
        weights_init(model, verbose=(local_rank == 0))
    if model_path != '':
        #------------------------------------------------------#
        #   权值文件请看README，百度网盘下载
        #------------------------------------------------------#
    ...
    if local_rank == 0:
        ...
        if total_step <= wanted_step:
            ...
            print('', flush=True)
            print('[Warning] 使用%s优化器时，建议将训练总步长设置到%d以上。' % (optimizer_type, int(wanted_step)), flush=True)
            print('[Warning] 本次运行的总训练数据量为%d，Unfreeze_batch_size为%d，共训练%d个Epoch，计算出总训练步长为%d。'
                  % (num_train, Unfreeze_batch_size, UnFreeze_Epoch, total_step), flush=True)
            print('[Warning] 由于总训练步长为%d，小于建议总步长%d，建议设置总世代为%d。'
                  % (total_step, int(wanted_step), wanted_epoch), flush=True)
            print('', flush=True)
    ...
    if True:
        ...
        gen             = DataLoader(train_dataset, shuffle = shuffle, batch_size = batch_size, num_workers = workers, pin_memory=True,
                                    drop_last=True, collate_fn=yolo_dataset_collate, sampler=train_sampler, 
                                    worker_init_fn=partial(worker_init_fn, rank=rank, seed=seed))
        gen_val         = DataLoader(val_dataset  , shuffle = shuffle, batch_size = batch_size, num_workers = workers, pin_memory=True, 
