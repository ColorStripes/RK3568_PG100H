...
def fit_one_epoch(model_train, model, ema, yolo_loss, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch, cuda, fp16, scaler, save_period, save_dir, local_rank=0):
    ...
    if local_rank == 0:
        ...
        if (
            ...
            and eval_callback.eval_flag
            and (epoch + 1) % eval_callback.period == 0
            and len(eval_callback.accs) > 1
        ):
            print(
                'Eval Metrics: mAP=%.4f acc=%.4f recall=%.4f'
                % (eval_callback.maps[-1], eval_callback.accs[-1], eval_callback.recalls[-1])
            )
