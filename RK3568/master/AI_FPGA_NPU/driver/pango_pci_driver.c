...
// #define MAX_CAM_BUFFS 2 
static CAM_CONFIG g_cam_config = {
    .cam_size = 0x3f4800,
    .read_size = 0x3f4800,
    .buff_count = 2,
    .cam_buffer_addrs = {0x00000000, 0x003F4800, 0, 0} // 默认两个地址
};
...
// 写入总传输字节数（控制器自动分chunk，只在最后完成时发一次MSI）
...
// 全局 DMA 缓冲的分配者 fd: 仅该 fd 的 release 才回收缓冲,
// 复用映射的其他 fd 关闭时不得误杀共享的 TX/RX 缓冲。
static struct file *g_dma_owner_filp = NULL;
long pango_cdev_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd)
	{
		...
		if (ret_r == 0) {
			...
			// TLP 长度 = min(total_length, MAX_TRANSFER_SIZE)
			...
			//  先写 DDR3 地址和长度，再写 TOTAL_LENGTH（触发控制器开始自动 chunk 循环）
			//  DDR3 地址由 total_length 推导，不再需要单独写入 ddr3_length
		...
		if (ret_w == 0) {
			...
			// TLP 长度 = min(total_length, MAX_TRANSFER_SIZE)
			// DMA 引擎直接用 o_req_length 作为传输 DW 数
			// total_size >= MAX_TRANSFER_SIZE 时，cap 到引擎最大能力，避免引擎超限死锁
			dma_info.cmd.data.length = (dma_operation.total_length >= MAX_TRANSFER_SIZE) ?
                               (MAX_TRANSFER_SIZE / 4) : (dma_operation.total_length / 4);
			set_ddr3_addr(dma_operation.ddr3_addr, pci_pango);
		...
		case PCI_UMAP_ADDR_CMD:
			...
			// TX/RX 全部释放后清空分配者, 防止其 fd 后续 close 时重复释放
			if (!dma_info.addr_r.data_buf && !dma_info.addr_w.data_buf)
				g_dma_owner_filp = NULL;
			spin_unlock(&dma_info.addr_r.lock);
		break;
		////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		case PCI_SET_CAM_CONFIG:
        {
            // 从用户态拷贝配置结构体到内核全局变量中
            if (copy_from_user(&g_cam_config, (CAM_CONFIG *)arg, sizeof(CAM_CONFIG))) {
                printk(KERN_ERR "pango_cdev: Failed to copy CAM_CONFIG from user space!\n");
                up(&pci_pango->_sem);
                return -EFAULT;
            }
            
            // 校验一下用户传进来的 buff_count 是否合法
            if (g_cam_config.buff_count == 0 || g_cam_config.buff_count > MAX_SUPPORTED_CAM_BUFFS) {
                printk(KERN_ERR "pango_cdev: Invalid buff_count %d. Resetting to 2.\n", g_cam_config.buff_count);
                g_cam_config.buff_count = 2; // 给一个安全的兜底值
            }

            printk(KERN_INFO "pango_cdev: CAM_CONFIG updated! cam_size: 0x%x, buff_count: %d\n", 
                   g_cam_config.cam_size, g_cam_config.buff_count);
        }
        break;
		...
		case PCI_GET_NPU:
        {
            ...
            // 等待 NPU 推理完成中断 (MSI #2 → g_npu_done++)
            ret_npu = wait_event_interruptible_timeout(g_msi.queue, 
                                                       atomic_read(&g_npu_done) > 0, 
                                                       msecs_to_jiffies(6000));  // NPU 推理最长 6 秒
            if (ret_npu <= 0) {
                if (ret_npu == 0)
                    printk(KERN_ERR "PCI_GET_NPU Error: Timeout waiting for NPU completion (60s) [g_npu_done=%d, g_dma_flag=%d, g_cam_flag=%d, g_ack_num=%d]\n",
                           atomic_read(&g_npu_done), atomic_read(&g_dma_flag), atomic_read(&g_cam_flag), atomic_read(&g_ack_num));
                else
                    printk(KERN_ERR "PCI_GET_NPU Error: Interrupted by signal while waiting for NPU [g_npu_done=%d, g_dma_flag=%d, g_cam_flag=%d, g_ack_num=%d]\n",
                           atomic_read(&g_npu_done), atomic_read(&g_dma_flag), atomic_read(&g_cam_flag), atomic_read(&g_ack_num));
                
                up(&pci_pango->_sem);
                return (ret_npu == 0) ? -ETIMEDOUT : -ERESTARTSYS;
            }
            atomic_dec(&g_npu_done);
        }
		break;
		...
		case GPIO_CTRL:
        {
            ...
            // 1. 从用户态安全地读取 C_GPIO 结构体数据
            if (copy_from_user(&c_gpio, (C_GPIO *)arg, sizeof(C_GPIO))) {
                printk(KERN_ERR "pango_cdev: Failed to copy C_GPIO from user space!\n");
                up(&pci_pango->_sem);
                return -EFAULT;
            }

            // 2. 纯粹的 GPIO 动态电平控制
            ret = gpio_direction_output(c_gpio.gpio_num, c_gpio.level);
            if (ret < 0) {
                printk(KERN_ERR "pango_cdev: Set GPIO %d output %d failed!\n", c_gpio.gpio_num, c_gpio.level);
                up(&pci_pango->_sem);
                return ret;
            }
            
            printk(KERN_INFO "pango_cdev: GPIO %d set to %s\n", 
                   c_gpio.gpio_num, 
                   c_gpio.level ? "HIGH (3.3V)" : "LOW (0V)");
        }
        break;
        ...
        case PWM_CTRL:
        {
            ...
            // 1. 从用户态安全地读取 C_GPIO 结构体数据
            if (copy_from_user(&c_gpio, (C_GPIO *)arg, sizeof(C_GPIO))) {
                printk(KERN_ERR "pango_cdev: Failed to copy PWM_CTRL from user space!\n");
                up(&pci_pango->_sem);
                return -EFAULT;
            }

            // 安全校验：防止数组越界
            if (c_gpio.gpio_num >= MAX_PWM_CHANNELS) {
                printk(KERN_ERR "pango_cdev: PWM channel %d out of range!\n", c_gpio.gpio_num);
                up(&pci_pango->_sem);
                return -EINVAL;
            }

            // 从我们定义的全局数组中获取该通道对应的 PWM 设备指针
            pwm_dev = g_pwm_devs[c_gpio.gpio_num];

            // 2. 专属的 PWM 控制逻辑
            if (c_gpio.pwm_enable) {
                
                // 如果指针为空，说明是第一次开启，需要向系统申请该 PWM 通道
                if (pwm_dev == NULL) {
                    pwm_dev = pwm_request(c_gpio.gpio_num, "pango_pwm_ctrl");
                    if (IS_ERR(pwm_dev)) {
                        printk(KERN_ERR "pango_cdev: Failed to request PWM channel %d\n", c_gpio.gpio_num);
                        up(&pci_pango->_sem);
                        return PTR_ERR(pwm_dev);
                    }
                    // 申请成功后，保存指针到数组中，以便下次使用
                    g_pwm_devs[c_gpio.gpio_num] = pwm_dev;
                }

                // 配置 PWM 周期和占空比 (注意：这里的单位必须是纳秒 ns)
                ret = pwm_config(pwm_dev, c_gpio.pwm_duty, c_gpio.pwm_period);
                if (ret) {
                    printk(KERN_ERR "pango_cdev: Failed to configure PWM %d\n", c_gpio.gpio_num);
                    up(&pci_pango->_sem);
                    return ret;
                }
                ...
                if (ret) {
                    printk(KERN_ERR "pango_cdev: Failed to enable PWM %d\n", c_gpio.gpio_num);
                    up(&pci_pango->_sem);
                    return ret;
                }

                printk(KERN_INFO "pango_cdev: PWM mode ENABLED on ch %d (Period: %d ns, Duty: %d ns)\n", 
                       c_gpio.gpio_num, c_gpio.pwm_period, c_gpio.pwm_duty);
                ...
                if (pwm_dev != NULL) {
                    ...
                    // 将数组中的指针清空，防止出现野指针
                    g_pwm_devs[c_gpio.gpio_num] = NULL; 
                    
                    printk(KERN_INFO "pango_cdev: PWM mode DISABLED on ch %d\n", c_gpio.gpio_num);
                } 
				else {
                    // 如果本来就没开启，给个提示即可
                    printk(KERN_WARNING "pango_cdev: PWM on ch %d was already disabled or not requested\n", c_gpio.gpio_num);
                }
            }
        }
		break;
...
{
	// 每次打开设备节点时，清空之前累积的废弃中断和状态
    // 防止 g_cam_done 堆积导致再次启动时 wait_event 瞬间穿透
...
int pango_cdev_release(struct inode *inode, struct file *filp)
{
    ...
    // 仅分配者 fd 的 release 才回收全局缓冲: 复用映射的其他 fd (如 HDMI 实例)
    // 关闭时不得释放共享的 TX/RX 缓冲, 否则会误杀仍在使用的相机/NPU 管线。
    if (g_dma_owner_filp == filp) {
        if (dma_info.addr_r.data_buf) {
            dma_free_coherent(&op_dev->dev, g_dma_alloc_size_r, dma_info.addr_r.data_buf, dma_info.addr_r.addr);
            dma_info.addr_r.data_buf = NULL;
        }
	...
	// === 新增：兜底释放所有可能被遗留占用的 PWM 通道 ===
    for (int i = 0; i < MAX_PWM_CHANNELS; i++) {
        if (g_pwm_devs[i] != NULL) {
            pwm_disable(g_pwm_devs[i]);
            pwm_free(g_pwm_devs[i]);
            g_pwm_devs[i] = NULL;
            printk(KERN_INFO "pango_cdev: Auto-released PWM channel %d on exit\n", i);
        }
    }
...
void read_img(void)
{
    ...
    target_addr = g_cam_config.cam_buffer_addrs[curr_idx];;
    ...
    // 使用结构体中的大小计算偏移和传输长度
    dma_operation.offset_addr = g_cam_config.cam_size * curr_idx;
    dma_operation.total_length = g_cam_config.read_size;
    ...
    // 使用取模运算保证循环
    atomic_set(&read_img_num, (curr_idx + 1) % g_cam_config.buff_count);
...
void read_npu(void)
{
    ...
    // 1. 配置 DMA 寄存器：三头连续数据从 FPGA DDR3 → 内核 RX 缓冲
    dma_operation.ddr3_addr   = NPU_OUTPUT_DDR3_BASE;
    dma_operation.offset_addr = g_cam_config.cam_size * g_cam_config.buff_count;
    dma_operation.total_length = NPU_TOTAL_OUTPUT_SIZE;  // = (20²+40²+80²) × 32CH
    
    dma_info.cmd.data.op_type = 1; // 写操作：FPGA 写入 内核

    // 2. TLP 长度，cap 到引擎最大能力避免超限
    dma_info.cmd.data.length = (dma_operation.total_length >= MAX_TRANSFER_SIZE) ?
                               (MAX_TRANSFER_SIZE / 4) : (dma_operation.total_length / 4);
...
static irqreturn_t pango_irq_handler1(int irq, void *dev_id)
{
	...
	// 【修复 2.1】如果 DMA 内存已被释放或尚未映射，说明 App 没在运行，直接丢弃该中断
    if (!dma_info.addr_w.data_buf) {
        return IRQ_HANDLED;
    }
	// 尝试抢占 DMA 权限
