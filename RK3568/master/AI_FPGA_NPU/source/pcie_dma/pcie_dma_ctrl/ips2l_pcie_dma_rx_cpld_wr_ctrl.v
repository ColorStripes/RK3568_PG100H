...
begin
    if(!rst_n)
        ...
        tag_addr_next <= i_multicpld_flag ? multicpld_addr + {i_cpld_length[7:0],2'b0} : 10'b0; //128byte
end

assign cpld_addr = (i_cfg_max_rd_req_size == 3'd0) ? {51'b0,{cpld_tag_use_ff,multicpld_addr[6:0]}} :  //128byte
                   (i_cfg_max_rd_req_size == 3'd1) ? {50'b0,{cpld_tag_use_ff,multicpld_addr[7:0]}} :  //256byte
                   (i_cfg_max_rd_req_size == 3'd2) ? {49'b0,{cpld_tag_use_ff,multicpld_addr[8:0]}} :  //512byte
                   (i_cfg_max_rd_req_size == 3'd3) ? {48'b0,{cpld_tag_use_ff,multicpld_addr[9:0]}} :  //1024byte
                                                     {51'b0,{cpld_tag_use_ff,multicpld_addr[6:0]}} ;

//******************************************************************************************
reg i_cpld_wr_start_reg;
always @(posedge clk or negedge rst_n) begin
    if(!rst_n)
        i_cpld_wr_start_reg <= 1'b0;
    else
        i_cpld_wr_start_reg <= i_cpld_wr_start;
end

reg [9:0] i_cpld_length_reg;
always @(posedge clk or negedge rst_n) begin
    if(!rst_n)
        i_cpld_length_reg <= 'b0;
    else
        i_cpld_length_reg <= i_cpld_length;
end

reg [7:0] cpld_dwbe_reg;
always @(posedge clk or negedge rst_n) begin
    if(!rst_n)
        cpld_dwbe_reg <= 'b0;
    else
        cpld_dwbe_reg <= cpld_dwbe;
end

reg [127:0] i_cpld_data_reg;
always @(posedge clk or negedge rst_n) begin
    if(!rst_n)
        i_cpld_data_reg <= 'b0;
    else
        i_cpld_data_reg <= i_cpld_data;
end

reg [3:0] i_cpld_dw_vld_reg;
always @(posedge clk or negedge rst_n) begin
    if(!rst_n)
        i_cpld_dw_vld_reg <= 'b0;
    else
        i_cpld_dw_vld_reg <= i_cpld_dw_vld;
end

// reg [63:0] cpld_addr_reg;
// always @(posedge clk or negedge rst_n) begin
//     if(!rst_n)
//         cpld_addr_reg <= 'b0;
//     else
//         cpld_addr_reg <= cpld_addr;
// end

reg [1:0] i_bar_hit_reg;
always @(posedge clk or negedge rst_n) begin
    if(!rst_n)
        i_bar_hit_reg <= 'b0;
    else
        i_bar_hit_reg <= i_bar_hit;
end
...
(
    ...
    //**********************************************************************
    .i_wr_start         (i_cpld_wr_start_reg    ),
    .i_length           (i_cpld_length_reg      ),
    .i_dwbe             (cpld_dwbe_reg          ),
    .i_data             (i_cpld_data_reg        ),
    .i_dw_vld           (i_cpld_dw_vld_reg      ),
    .i_addr             (cpld_addr          ),
    .i_bar_hit          (i_bar_hit_reg          ),
