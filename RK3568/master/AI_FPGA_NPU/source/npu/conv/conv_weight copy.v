...
(
    ...
    // wire [AXI_ADDR_WIDTH-1 : 0] weight_stride = type ? WEIGHT_STRIDE_11 * multiple : WEIGHT_STRIDE_33 * multiple;
    ...
    always @(posedge clk) begin
        if(type) begin
            ...
            weight_stride <= WEIGHT_STRIDE_33 * multiple;
        end
    end
    ...
    always @(posedge clk) begin
        if(rst) begin
            ...
            if(raddr == DEPTH_ADDR) begin
                ...
                for(k = 0; k < RAM_CNT; k = k + 1) begin
                    if(r_ram_cnt == (+1)) begin
                        if( ((w_ram_cnt != (k+1)) && ~empty[k]) || ((w_ram_cnt == (k+1)) && almost_full[k]) ) begin
                            ren[k] <= ren_all;
                        end
                    end
                end
