module conv11_para_top #(
    ...
    parameter CHA_PAR_IN = 16,                           //输入通道并行度
            ...
            //   CHA_OUT_WIDTH = $clog2(CHA_IMG_OUT),
              ...
              MAX_OUT_LEN = 10240,                       //此模块所接受的最大字节数  也就是一行*所有输出通道的字节数   不是！最大通道*最大行数=最大长度
...
(
    ...
    output [DATA_WIDTH_IN-1 : 0]         m_data      ,
    output                               m_valid     ,
    output                               m_last      ,
    input                                m_ready     ,
...
///////////////////////////////////////// 计算所需数据的算子级别寄存器 /////////////////////////////////
    ...
    reg  [OUT_LEN_WIDTH : 0]           out_col_channel_num_reg;
...
ctrl(
    ...
    //from out_buf
    .calculate_req(add_m_req)   ,            //说明计算模块可以接受新的计算数据 请求给数据
    //to add
    ...
    .calculate_end(calculate_weight_end)
);
...
weight_bias(
    ...
    // .weight_len(weight_len_reg),  
    .bias_len(weight_len_reg),
...
out_para_buf_top_inst(
    .clk(clk)              ,
    .rst(rst)              ,

    .start (start_reg )           ,
    .type  (type_reg  )           ,
    .stride(1'b0)           ,                     //0为1步长 1为2步长
    
    .base_addr(m_addr_reg)                        ,
    .out_col_channel_num(out_col_channel_num_reg) , //out_col_channel_num = col * channel
    .row_num(row_num_reg)                         ,
