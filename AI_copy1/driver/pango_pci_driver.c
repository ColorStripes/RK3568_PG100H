...
#include "id_config.h"
#include <linux/gpio.h> // 引入内核 GPIO 控制头文件
//#define NDEBUG
...
long pango_cdev_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd)
	{
		...
		case GPIO_OUT:
        {
            int gpio_num = 91; // 之前算好的 GPIO1 编号

            // 设置方向为输出，并初始化输出高电平（1）
            int ret = gpio_direction_output(gpio_num, 1);
            if (ret < 0) {
                printk(KERN_ERR "pango_cdev: Set GPIO1 output high failed!\n");
                up(&pci_pango->_sem);
                return ret;
            }
            
            printk(KERN_INFO "pango_cdev: GPIO1 set to HIGH (3.3V)\n");
        }
		break;
        ...
        case GPIO_IN:
        {
            ...
            // 按照你的要求：调用 GPIO_IN 时输出低电平（0）
            int ret = gpio_direction_output(gpio_num, 0);
            if (ret < 0) {
                printk(KERN_ERR "pango_cdev: Set GPIO1 output low failed!\n");
                up(&pci_pango->_sem);
                return ret;
            }

            printk(KERN_INFO "pango_cdev: GPIO1 set to LOW (0V)\n");

            /*  友情提示：如果你以后想把 GPIO_IN 改回它原本的意思（切换为输入模式并读取电平），
               可以用下面这段注释掉的代码：
            
            int val;
            gpio_direction_input(gpio_num); // 切换为输入模式
            val = gpio_get_value(gpio_num);  // 读取外部电平（0或1）
