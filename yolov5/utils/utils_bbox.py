...
class DecodeBox():
    ...
    def agnostic_nms(self, detections, nms_thres):
        """跨类别 NMS：重叠框只保留置信度最高的一个。"""
        ...
        while len(order) > 0:
            ...
            if len(order) == 1:
                break
            rest = order[1:]
            ious = self._bbox_iou_numpy(boxes[i], boxes[rest])
            order = rest[ious < nms_thres]
        return detections[np.array(keep)]

    @staticmethod
    def _bbox_iou_numpy(box, boxes):
    ...
    def non_max_suppression(self, prediction, num_classes, input_shape, image_shape, letterbox_image, conf_thres=0.5, nms_thres=0.4, agnostic=False):
        #----------------------------------------------------------#
        ...
        for i, image_pred in enumerate(prediction):
            ...
            # conf_mask_npy = conf_mask.numpy()
            #----------------------------------------------------------#
            #   根据置信度进行预测结果的筛选
            #----------------------------------------------------------#
            # image_pred_npy = image_pred.numpy()
            ...
            if output[i] is not None:
                ...
                if agnostic:
                    output[i] = self.agnostic_nms(output[i], nms_thres)
        return output
